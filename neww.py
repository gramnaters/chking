...
def step1_add_to_cart(session):
    ...
    session_token, checkout_token = _follow_checkout_redirects(session, checkout_url)
    
    if checkout_token:
        print(f"  [OK] Checkout token: {checkout_token}")
        if session_token:
            print(f"  [OK] Session token extracted from redirect JWT")
        return checkout_token, session_token, session.cookies
    
    return None, None, None

def _follow_checkout_redirects(session, checkout_url, timeout=None):
    import base64 as _b64
    from urllib.parse import parse_qs as _pqs, urlparse as _up, unquote as _uq
    if timeout is None:
    ...
    try:
        r = session.get(checkout_url, allow_redirects=False, timeout=timeout, verify=False)
    except Exception:
        try:
            r = session.get(checkout_url, allow_redirects=True, timeout=timeout, verify=False)
            if '/checkouts/cn/' in r.url:
                checkout_token = r.url.split('/checkouts/cn/')[1].split('/')[0].split('?')[0]
                session_token = extract_session_token(r.text)
            return session_token, checkout_token
        except Exception:
            return None, None
    ...
    for _ in range(max_redirects):
        ...
        if loc.startswith('/'):
            parsed_prev = _up(r.url if hasattr(r, 'url') else checkout_url)
            loc = f"{parsed_prev.scheme}://{parsed_prev.netloc}{loc}"

        parsed = _up(loc)
        params = _pqs(parsed.query)

        if 'token' in params and 'shop.app' in parsed.netloc:
            try:
                jwt_token = params['token'][0]
                jwt_parts = jwt_token.split('.')
                if len(jwt_parts) >= 2:
                    payload_b64 = jwt_parts[1] + '=' * (4 - len(jwt_parts[1]) % 4)
                    jwt_data = json.loads(_b64.urlsafe_b64decode(payload_b64))
                    if 'session_token' in jwt_data:
                        session_token = jwt_data['session_token']
            except Exception:
                pass

            back_url = params.get('ur_back_url', [None])[0]
            if back_url:
                back_url = _uq(back_url)
                if '/checkouts/cn/' in back_url:
                    checkout_token = back_url.split('/checkouts/cn/')[1].split('/')[0].split('?')[0]
                try:
                    r = session.get(back_url, timeout=timeout, verify=False)
                except Exception:
                    pass
                break
            else:
        ...
        try:
            r = session.get(loc, allow_redirects=False, timeout=timeout, verify=False)
        except Exception:
            break

        if '/checkouts/cn/' in loc and not checkout_token:
            checkout_token = loc.split('/checkouts/cn/')[1].split('/')[0].split('?')[0]

    if not checkout_token and hasattr(r, 'url') and '/checkouts/cn/' in r.url:
        checkout_token = r.url.split('/checkouts/cn/')[1].split('/')[0].split('?')[0]

    if not session_token and hasattr(r, 'text'):
        session_token = extract_session_token(r.text)
...
def extract_session_token(html):
    ...
    if meta_match:
        ...
        token = content.strip('"')
        if len(token) > 50:
            print(f"  [OK] Session token extracted (legacy meta tag)")
            return token
...
def get_delivery_line_config(shipping_handle="any", destination_changed=True, merchandise_stable_id=None, use_full_address=False, phone_required=False):
    ...
    config = {
        ...
        "selectedDeliveryStrategy": selected_strategy,
        ...
        try:
            ...
            if typename == 'FilledDeliveryExpectationTerms':
                ...
                if delivery_expectations:
                    ...
                    print(f"  [INFO] Expectations filled but empty - polling...")
                    polled_data = poll_proposal_ctx(
                        session, checkout_token, session_token, merchandise_stable_id,
                        shipping_handle, shop_url, variant_id, phone_required, shipping_amount
                    )
                    if polled_data is not None:
                        q2, e2 = (polled_data[0], polled_data[1]) if len(polled_data) >= 2 else (None, [])
                        t2 = polled_data[2] if len(polled_data) >= 3 else None
                        if q2: queue_token = q2
                        if t2: actual_total = t2
                        if e2: delivery_expectations = e2; print(f"  [OK] Poll gave {len(e2)} expectations")
            elif typename == 'PendingTerms':
                print(f"  [INFO] Expectations pending...")
                if delivery_typename == 'PendingTerms':
                    print(f"  [INFO] Both delivery and expectations pending - using comprehensive poll")
